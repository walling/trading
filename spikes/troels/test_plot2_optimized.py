...
def get_row_groups_for_market(metadata, market_id):
    ...
    for r, row_group in iter_row_groups(metadata):
        ...
        assert column.statistics.has_min_max, \
            'Market id statistics not saved'
        assert column.statistics.min == column.statistics.max, \
            'Market id not constant within row group'
        if column.statistics.min == market_id:
            result.append(r)
    return result
...
def get_price_scale(metadata, row_group_id):
    ...
    assert column.statistics.has_min_max, \
        'Price scale statistics not saved'
    assert column.statistics.min == column.statistics.max, \
        'Price scale not constant within row group'
